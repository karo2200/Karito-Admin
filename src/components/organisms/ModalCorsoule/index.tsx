...
import { useCarousel_GetAllQuery, useCity_SetActiveCarouselMutation } from 'src/graphql/generated';
...
function MyModal({ open, handleClose, name, handleReject, cityId }) {
	...
	const { mutate: mutateCity, isLoading: isLoading } = useCity_SetActiveCarouselMutation();
	const { data, isSuccess, isError } = useCarousel_GetAllQuery(
		{
			skip: 0,
			take: 1000,
		},
	...
	useEffect(() => {
		if (isSuccess) {
			const newList = [
				...
				...(data?.carousel_getAll?.result?.items?.map(({ title, id }) => ({
					option: title,
					value: id,
				})) || []),
			];
