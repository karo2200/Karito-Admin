		...
		>
			...
			<Grid container spacing={2} sx={{ marginTop: '5px', borderBottom: '1px solid #DEE2E6' }}>
				<Grid item xs={12} sx={{ flexBasis: '90px !important', flexGrow: 0, flexShrink: 0 }}>
					<img src={data?.specialist?.profileImageUrl} style={{ width: '80px', height: '80px', borderRadius: 5 }}></img>
				</Grid>
				<Grid item xs={12} sm={4}>
					<Typography sx={{ color: COLORS.textcolor, marginBottom: '5px' }}>
						{data?.specialist?.firstName + ' ' + data?.specialist?.lastName}
					</Typography>
					<Rating name="half-rating-read" defaultValue={data?.averageRating} precision={0.5} readOnly dir="ltr" />
					<Typography sx={{ color: COLORS.textcolor, marginBottom: '5px' }}>
						{'تعداد رای دهندگان: ' + data?.specialist?.rateCount}
					</Typography>
				</Grid>
				<Grid item xs={12} sm={4}>
					<Typography sx={{ color: COLORS.textcolor, marginBottom: '5px' }}>
						{' موبایل : ' + data?.specialist?.phoneNumber}
					</Typography>
					<Typography sx={{ color: COLORS.textcolor, marginBottom: '5px' }}>
						{'سرویس: ' + data?.serviceType?.name}
					</Typography>
				</Grid>
			</Grid>
			<Grid container spacing={2} sx={{ marginTop: '5px', borderBottom: '1px solid #DEE2E6' }}>
				<Grid item xs={12} sx={{ flexBasis: '90px !important', flexGrow: 0, flexShrink: 0 }}>
					<img src={data?.customer?.profileImageUrl} style={{ width: '80px', height: '80px', borderRadius: 5 }}></img>
				</Grid>
				<Grid item xs={12} sm={4}>
					<Typography sx={{ color: COLORS.textcolor, marginBottom: '5px' }}>
						{data?.customer?.firstName + ' ' + data?.customer?.lastName}
					</Typography>
					<Typography sx={{ color: COLORS.cancelHover, marginBottom: '5px' }}>مشتری</Typography>
				</Grid>
				<Grid item xs={12} sm={6}>
					<Typography sx={{ color: COLORS.textcolor, marginBottom: '5px' }}>
						{' موبایل : ' + data?.customer?.phoneNumber}
					</Typography>
					<Typography sx={{ color: COLORS.textcolor, marginBottom: '5px' }}>
						{'آدرس: ' +
