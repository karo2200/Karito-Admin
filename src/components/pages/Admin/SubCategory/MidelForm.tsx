...
import { useServiceSubCategory_DeleteMutation } from 'src/graphql/generated';
...
const Index: FC<IPageProps> = ({ DataRow, OnhandleEditClick, onRefreshItem }) => {
	...
	const { mutate: mutateState, isLoading: isLoading } = useServiceSubCategory_DeleteMutation();

	const handleConfirm = async () => {
		await mutateState(
			{
				input: {
					serviceSubCategoryId: idstate,
				},
			},
			{
				onSuccess: async (res) => {
